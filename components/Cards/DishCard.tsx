...
interface DishCardProps {
  image: any; // Thay đổi type nếu cần
  name: string;
  rating: number;
  ratingCount: number;
  type: string;
...
const DishCard: React.FC<DishCardProps> = ({
  ...
  return (
    <View className="pt-24 m-2 overflow-hidden relative ">
      <Image
        source={image}
        className="absolute top-2 z-10 left-[20%] transform -translate-x-1/2 h-[130px] w-[130px] rounded-full border- p-2 border-black"
        resizeMode="cover"
      />
      <View className="pt-14 rounded-[16px] z-0 shadow-xl bg-[#FFF1E1]">
        ...
        <TouchableOpacity className="border-[#E45834] border-2 p-2 rounded-[20px]  w-[70%] mx-auto mb-4 mt-2">
